...
if not isfile(join(FRAMEWORK_DIR,mcu, "lib", ".patched")):
    ...
    if mcu != "esp32p4":
        # Resolve objcopy from the installed platform package. Recent pioarduino
        # releases renamed toolchain packages, so `pio pkg exec` with a legacy
        # package name is not portable. At pre-script time $OBJCOPY is also not
        # populated yet.
        if mcu in ("esp32c5", "esp32c6"):
            toolchain_package = "toolchain-riscv32-esp-elf"
            objcopy_name = "riscv32-esp-elf-objcopy"
        else:
            toolchain_package = "toolchain-xtensa-esp-elf"
            objcopy_name = "xtensa-esp-elf-objcopy"

        toolchain_dir = env.PioPlatform().get_package_dir(toolchain_package)
        objcopy = join(toolchain_dir, "bin", objcopy_name) if toolchain_dir else ""
        if not isfile(objcopy):
            raise RuntimeError("Required objcopy not found: %s" % objcopy)

        result = env.Execute(
            "%s --weaken-symbol=ieee80211_raw_frame_sanity_check %s %s"
            % tuple(map(shlex.quote, (objcopy, original_file, patched_file)))
        )
        if result != 0 or not isfile(patched_file):
            raise RuntimeError("Failed to patch %s" % original_file)

        if isfile("%s.old" % (original_file)):
            remove("%s.old" % (original_file))
        if not isfile(original_file):
            raise RuntimeError("Original file not found: %s" % original_file)
