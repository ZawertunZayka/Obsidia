...
Target: photographed passive eight-pin microSD SPI/SDIO breakout. Its header
labels are `GND, DO2/NC, DO3/CS, CMD/MOSI, CLK, DO/MISO, DO1/NC, VCC`. Connect
`VCC` directly to board `3V3`; this board has no input regulator or level
shifter. `DO2` and `DO1` remain unconnected in SPI mode. Do not connect any pin
to 5 V.

The earlier blue six-pin adapter with an onboard 1117-class regulator and
LVC125A-like buffer is quarantined: it held MISO low even with no card fitted.
It is no longer the target of this diagnostic.
