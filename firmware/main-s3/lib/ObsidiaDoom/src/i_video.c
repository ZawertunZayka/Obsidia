...
{
	unsigned char *scr=(unsigned char*)screens[0].data;
#if 0
	int x, y;
	char *chrs=" '.~+mM@";
	ets_printf("\033[1;1H");
...
#endif
#if 1
	obsidia_doom_present(scr, lcdpal);
#endif
	//Flip framebuffers
...
void I_PreInitGraphics(void)
{
	...
	screenbuf=heap_caps_malloc(320*240, MALLOC_CAP_SPIRAM|MALLOC_CAP_8BIT);
	if (!screenbuf) screenbuf=heap_caps_malloc(320*240, MALLOC_CAP_INTERNAL|MALLOC_CAP_8BIT);
	assert(screenbuf);
#endif
}
